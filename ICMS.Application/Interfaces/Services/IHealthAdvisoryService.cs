...
namespace ICMS.Application.Interfaces.Services
{
    public interface IHealthAdvisoryService
    {
        ...
        Task<HealthAdvisoryDetailsDto> ResendAsync(int id, CancellationToken ct = default);
        Task<(byte[] content, string contentType, string fileName)> GetImageAsync(int id, CancellationToken ct = default);
